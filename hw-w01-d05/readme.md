...
## Completion
There are 12 steps to the homework. A complete homework requires at least 6 complete steps not counting Part 1 which is provided. If you can only complete 6, don't worry - we'll be revisiting this homework soon when we learn more about DOM manipulation.

If you decide to work past the first 7 steps, be sure to review documentation, research on Stack Overflow - there may be content we haven't covered in class.

After each step is completed, remember to `git add .` and to `git commit -m "meaningful commit"`! When you're finished, make sure you `git push origin master` to your repo and submit your assignment on Schoology as usual.
